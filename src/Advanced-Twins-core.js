...
export class KeyObserver {
    ...
    async executeCommand(action, jikanwariShozokuCode, yobi, jigen, jikanwariCode) {
        if (action === 'Add') {
        ...
        const finalizeAction = async () => {
          try {
                    await this.sendRequest('back', this.getButtonConfig('back'));
        ...
        if (action === 'Add') {
              try {
                    await this.sendRequest('input', { ...config.inputParams, jikanwariCode });
                    await this.sendRequest('insert', { nendo: new Date().getFullYear().toString(), jikanwariCode });
              ...
              try {
                    await this.sendRequest('delete', { ...config, jikanwariCode });
                    await this.sendRequest('delete', { _flowExecutionKey: this.latestKey || this.currentKey });
                    await finalizeAction();
              } catch (error) {
...
export class AdvancedSyllabus {
    ...
    injectStyles() {
        ...
        linkElement.href = 'ui/styles.css';
    ...
    async addLinksToTimetable() {
        ...
        try {
              ...
              for (const cell of timetableCells) {
                    await this.addLinkToCell(cell, false);
                    await this.addDeleteButton(cell);
              }
              ...
              for (const row of concentratedRows) {
                    ...
                    if (syllabusCell) {
                          await this.addLinkToCell(syllabusCell, true);
                          await this.addDeleteButton(syllabusCell);
                    }
              }
        } catch (error) {
      ...
      async init() {
            try {
                  ...
                  if (targetIframe && this.keyObserver) {
                        const jsonData = await ApiClient.fetchJsonData(this.urls[this.currentUrlIndex]);
                        this.createUI();
...
async handleSubjectButtonClick(subjectCode, subjectName, action) {
    if (!this.keyObserver) {
    ...
    if (action === 'add') {
      try {
        ...
        await this.keyObserver.executeCommand('Add');
      } catch (error) {
